            ...
            <div class="h-60 bg-white"
                ...
                <div class="row pl-3 pt-3 border-bottom m-0 p-0 bg-white">
                    <div class="col-12 px-0">
                        <h6 class="font-weight-semibold m-0 text-secondary">{{ customer.description }}</h6>
                    </div>
                ...
                <template v-for="(item,index) in form.items">
                    <div :key="index"
                        ...
                        <div class="col-2 p-r-0 m-l-2">
                            <small class="font-weight-semibold m-0 text-secondary">{{ item.quantity }}</small>

                        </div>
                        <div class="col-6 px-0">
                            <small class="font-weight-semibold m-0 m-b-0 text-secondary">{{ item.item.description }}</small>
                        ...
                        <div class="col-4 p-l-0">
                            ...
                            <small class="font-weight-semibold m-0 text-end text-secondary">
                                {{ currencyTypeActive.symbol }} {{ item.total }}</small>
                        </div>
        ...
        async clickPayment() 
        {
            ...
            else
            {
                if(this.form.total <= 0){
                    ...
                    let isAllAffectation15 = items.every(item => item.affectation_igv_type_id === '15');
                    if(!isAllAffectation15){
                        return this.$message.error('El total debe ser mayor a 0')
                    }
                }
            }
